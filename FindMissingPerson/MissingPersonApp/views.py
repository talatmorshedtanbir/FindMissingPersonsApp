...
def HelperView(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            model = KNeighborsClassifier(n_neighbors = 1, algorithm='ball_tree', weights='distance')
            # fdtraining of model
            ...
            while(True):
                ...
                for face_encoding in face_encodings:
                    ...
                    if(np.any(narr<0.4)):
                        ...
                        file_name = "VideoData/img" + "_" + labels[0]+"_"+labels[1] +"_" + ".jpg"
                        ...
                        client.messages.create(to='+8801817316436',from_='+14695303875',body="A person was found. Name is "+labels[1]+", Contact no is "+labels[0]+", Found location is "+loc)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for f in faces:
                x, y, w, h = f
                sub_face = image[y:y+h, x:x+w]
                file_name = "FoundData/img" + "_" + Person.objects.last().location + "_" + str(i)+str(Person.objects.last().phone) + ".jpg"
            ...
            npwriter.write(str(Person.objects.last().phone)+" , " + Person.objects.last().location, np.array(f_list))
            return redirect('home')
        else:
            print("Form Not Valid")
    else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if(str(Person.objects.last().child_pic1) != empty_file):
                images.append(face_recognition.load_image_file('media/'+str(Person.objects.last().child_pic1)))
            if(str(Person.objects.last().child_pic2) != empty_file):
                images.append(face_recognition.load_image_file('media/'+str(Person.objects.last().child_pic2)))
            if(str(Person.objects.last().child_pic2) != empty_file):
                images.append(face_recognition.load_image_file('media/'+str(Person.objects.last().child_pic2)))
                
            i=0
            for img in images:
                file_name = "FoundData/img" + "_" + Person.objects.last().location + str(i) +"_" + ".jpg"
                cv2.imwrite(file_name,img)
                i+=1
                list_of_face_encodings = np.array(face_recognition.face_encodings(img))
                npwriter2.write(str(Person.objects.last().phone)+" , " + Person.objects.last().location, np.array(list_of_face_encodings))
            return redirect('home')
        else:
            print("Form Not Valid")
    else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if(str(Person.objects.last().child_pic3) != empty_file):
                images.append(face_recognition.load_image_file('media/'+str(Person.objects.last().child_pic3)))
            if(str(Person.objects.last().child_pic4) != empty_file):
                images.append(face_recognition.load_image_file('media/'+str(Person.objects.last().child_pic4)))
            if(str(Person.objects.last().child_pic5) != empty_file):
                images.append(face_recognition.load_image_file('media/'+str(Person.objects.last().child_pic5)))
            i=0
            emp=[]
            facedata = "media/haarcascade_frontalface_default.xml"
            cascade = cv2.CascadeClassifier(facedata)
            for img in images:
                ...
                file_name = "FoundData/img" + "_" + Person.objects.last().location + str(i) +"_" + ".jpg"
                cv2.imwrite(file_name,sub_face)
                list_of_face_encodings = np.array(face_recognition.face_encodings(sub_face))
                if(list_of_face_encodings!=emp):
                    npwriter2.write(str(Person.objects.last().phone).replace('+','')+" , " + Person.objects.last().location, np.array(list_of_face_encodings))
                i+=1
            return redirect('home')
        else:
            print("Form Not Valid")
...
def PoliceView(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            image = face_recognition.load_image_file('media/'+str(Child.objects.last().child_pic))
            file_name = "MissingData/img" + "_" + Child.objects.last().name +"_" + ".jpg"
            cv2.imwrite(file_name,image)
            list_of_face_encodings = np.array(face_recognition.face_encodings(image))
            ...
            if(np.any(narr<0.4)):
                ...
                match =Match(name=Child.objects.last().name, contact=str(Child.objects.last().contact), location=labels[1], foundContact=labels[0])
                match.save()
                ...
                npwriter3.write(str(Child.objects.last().contact)+" , " + Child.objects.last().name, np.array(list_of_face_encodings))
...
def MatchView(request):
        ...
        for x in X:
            ...
            if(np.any(narr<=0.4)):
                ...
                match = Match(name=labs[1], contact=labs[0], location=vals[0], foundContact=vals[1])
                match.save()
...
def generate_otp(mobile_no):
	...
	auth_token ='976486b7e3e5d2286327b7147954b6e6'
	client = Client(account_sid, auth_token)
	client.messages.create(to=mobile_no,from_='+14695303875',body='Your One Time Password is '+str(otp))
	f=open('otp.txt','w')
	f.write(str(otp))
	f.close()	
